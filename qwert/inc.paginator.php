 ...
 */
 ...
 if(!isset($_pagi_nav_siguiente)){
        // Si no se ha elegido una cadena para el enlace "siguiente"
        // Se toma la cadena por defecto.
        $_pagi_nav_siguiente = "Pr&oacute;xima &raquo;";
 } 
 ...
 */
 ...
 if($_pagi_conteo_alternativo == false){
        ...
        $_pagi_result2 = $db->query($_pagi_sqlConta);
        // Si ocurrió error y mostrar errores está activado
        if($_pagi_result2 == false && $_pagi_mostrar_errores == true){
                die (" Error en la consulta de conteo de registros: $_pagi_sqlConta. Mysql dijo: <b>".$db->error()."</b>");
        }
        $_pagi_totalReg = $db->result($_pagi_result2,0,0);//total de registros
 } else{
        $_pagi_result3 = $db->query($_pagi_sql);
        // Si ocurrió error y mostrar errores está activado
        if($_pagi_result3 == false && $_pagi_mostrar_errores == true){
                die (" Error en la consulta de conteo alternativo de registros: $_pagi_sql. Mysql dijo: <b>".$db->error()."</b>");
        }
 ...
 */
 ...
 if ($_pagi_actual != 1){
        ...
        $_pagi_navegacion_temporal[] = "<a href=\"".$_pagi_enlace."_pagi_pg=".$_pagi_url."\" ".$_pagi_nav_estilo_mod.">$_pagi_nav_primera</a>";

        // Si no estamos en la página 1. Ponemos el enlace "anterior"
        $_pagi_url = $_pagi_actual - 1; //será el número de página al que enlazamos
        $_pagi_navegacion_temporal[] = "<a href=\"".$_pagi_enlace."_pagi_pg=".$_pagi_url."\" ".$_pagi_nav_estilo_mod.">$_pagi_nav_anterior</a>";
 }
 ...
 for ($_pagi_i = $_pagi_nav_desde; $_pagi_i<=$_pagi_nav_hasta; $_pagi_i++){//Desde página 1 hasta última página ($_pagi_totalPags)
        if ($_pagi_i == $_pagi_actual) {
                ...
                $_pagi_navegacion_temporal[] = "<strong>$_pagi_i</strong>";
        }else{
                // Si es cualquier otro. Se escibe el enlace a dicho número de página.
                $_pagi_navegacion_temporal[] = "<a href=\"".$_pagi_enlace."_pagi_pg=".$_pagi_i."\" ".$_pagi_nav_estilo_mod.">".$_pagi_i."</a>";
        }
 }
 ...
 if ($_pagi_actual < $_pagi_totalPags){
        ...
        $_pagi_url = $_pagi_actual + 1; //será el número de página al que enlazamos
        $_pagi_navegacion_temporal[] = "<a href=\"".$_pagi_enlace."_pagi_pg=".$_pagi_url."\" ".$_pagi_nav_estilo_mod.">$_pagi_nav_siguiente</a>";
        ...
        $_pagi_navegacion_temporal[] = "<a href=\"".$_pagi_enlace."_pagi_pg=".$_pagi_url."\" ".$_pagi_nav_estilo_mod.">$_pagi_nav_ultima</a>";
 }
 ...
 */
 ...
 $_pagi_result = $db->query($_pagi_sqlLim);
 // Si ocurrió error y mostrar errores está activado
 if($_pagi_result == false && $_pagi_mostrar_errores == true){
        die ("Error en la consulta limitada: $_pagi_sqlLim. Mysql dijo: <b>".$db->error()."</b>");
 }
